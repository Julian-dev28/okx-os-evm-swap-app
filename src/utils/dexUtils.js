...
const ethMainnet = 'https://eth-mainnet.g.alchemy.com/v2/I177iatNveGoBt3geurbwflbKjKh8bzq';
const ethSepolia = 'https://eth-sepolia.g.alchemy.com/v2/I177iatNveGoBt3geurbwflbKjKh8bzq';
...
export const fromTokenAddress = xlayerUSDC;
...
function getQuoteHeaders(quoteParams) {
    ...
    const stringToSign = timestamp + 'GET' + '/api/v5/dex/aggregator/quote?' + new URLSearchParams(quoteParams).toString();
...
function getApproveTransactionHeaders(params) {
    const date = new Date();
    const timestamp = date.toISOString();
    const stringToSign = timestamp + 'GET' + '/api/v5/dex/aggregator/approve-transaction?' + new URLSearchParams(params).toString();
...
export async function sendApproveTx(approveAmount) {
    const allowanceAmount = await getAllowance(user, spenderAddress, fromTokenAddress);
    if (BigInt(allowanceAmount) < BigInt(approveAmount)) {
        let gasPrice = await web3.eth.getGasPrice();
        let nonce = await web3.eth.getTransactionCount(user)
        const { data } = await approveTransaction(chainId, fromTokenAddress, approveAmount);
...
function getSwapHeaders(swapParams) {
    const date = new Date();
    const timestamp = date.toISOString();
    const stringToSign = timestamp + 'GET' + '/api/v5/dex/aggregator/swap?' + new URLSearchParams(swapParams).toString();
...
export async function sendSignedTransaction(txObject) {
    const { rawTransaction } = await web3.eth.accounts.signTransaction(txObject, privateKey);
